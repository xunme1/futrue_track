# -*- coding: utf-8 -*-
"""席位追踪每日编排（日更入口）：抓数 → 方向图 → 详情 JSON + AI 解读。
...
抓数失败退出码 1（阻断）；DeepSeek 失败仅警告（PNG/CSV/JSON 已产出，退出码 0）。
"""
...
def main(argv=None):
    ...
    ap.add_argument("--no-report", action="store_true", help="只出 CSV 和方向图，不调 DeepSeek")
    ...
    if csv.exists():
        ...
        print(f"[1/3] 缓存命中: {csv}（{len(df)} 行）")
    else:
        print(f"[1/3] 抓取 {start} ~ {target} 会员持仓（约 {40 * 1.2:.0f}s）…")
    ...
    md_path.write_text(
        ...
        encoding="utf-8")
    print(f"[3/3] AI 解读: {md_path}（{used_model}，{n_chars} 字）")

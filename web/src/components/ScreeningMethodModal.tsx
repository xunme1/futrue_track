  ...
  return (
    <div className="modal-mask" onClick={onClose}>
      <section className="modal-panel screening-method" onClick={(event) => event.stopPropagation()}>
        ...
        {timeframe === '1d' && <><h3>趋势榜排名变化</h3>
          <p>多头、空头趋势榜统一使用合约池最新交易日；缺当日 K 线的品种暂不参与这两张榜的排名。历史排名按当前合约池回算，每个交易日仅纳入当日有数据且处于对应趋势的品种，沿用上述分数和排序；同分按合约池顺序排列。</p>
          <p>连续两根 K 线处于同一趋势时，变化 = 上一根名次 − 当前名次。红色 ↑ 表示名次上升，绿色 ↓ 表示名次下降，→0 表示持平；其他品种进出榜也会影响名次。重新进入趋势显示“新入榜”，缺少可比数据或有效分数显示 —。</p>
          <p>悬停、键盘聚焦或点击箭头可查看最近 7 根 K 线的排名，第 1 名在上方。仅显示本次连续在榜段；入榜前留空，缺行情处断线，不跨缺口比较升降。历史使用当前版本的评分公式，不恢复过去已移除的合约。</p>
        </>}
        <h3>边界与颜色说明</h3>

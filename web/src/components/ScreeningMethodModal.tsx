      ...
      <section className="modal-panel screening-method" onClick={(event) => event.stopPropagation()}>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td>多转空</td><td>{timeframe === '4h' ? '最近 9 根存在多头支撑带；当前下跌蓝 K 的收盘价跌破该带下沿 EE。前 8 根含偏强红 K、或含 DSBE 减仓倒手指时，各加 1 个 🌟。' : '最近 8 根内由红 K 直接转为首根蓝 K；该根收盘价 &lt; EE；转折前为持多，之后蓝 K 连续至少 1 根，且最新 K 仍为蓝 K'}</td><td>{timeframe === '4h' ? '🌟 数降序，再按 score 升序' : '按最新 score 升序'}</td></tr>
            <tr><td>空转多</td><td>{timeframe === '4h' ? '最近 9 根存在空头压力带；当前上涨红 K 的收盘价突破该带上沿 PP。前 8 根含偏弱蓝 K、或含 SB 增仓笑脸时，各加 1 个 🌟。' : '最近 8 根内由蓝 K 直接转为首根红 K；该根收盘价 &gt; PP；转折前为持空，之后红 K 连续至少 1 根，且最新 K 仍为红 K'}</td><td>{timeframe === '4h' ? '🌟 数降序，再按 score 降序' : '按最新 score 降序'}</td></tr>
          </tbody>
        </table>
        ...
        {timeframe !== '4h' && <><h3>榜单预警类别</h3><ul><li><b>压力回踩</b>：持空时，最高价与收盘价均在 KK–PP 压力带内，收盘未上破 PP；表示压力仍有效，需留意反弹后的方向选择。</li><li><b>支撑回踩</b>：持多时，最低价与收盘价均在 EE–DD 支撑带内，且收盘严格高于下沿 EE；表示支撑仍有效，需留意反弹延续。</li></ul><p className="legend-note">报告中仍保留多转空、空转多两类转换预警，供策略分析使用；它们不显示在榜单预警区。</p></>}
      </section>
    </div>
  )
}

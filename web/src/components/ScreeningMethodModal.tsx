  ...
  return (
    <div className="modal-mask" onClick={onClose}>
      <section className="modal-panel screening-method" onClick={(event) => event.stopPropagation()}>
        ...
        {timeframe !== '4h' && <><h3>榜单预警类别</h3><ul><li><b>压力回踩</b>：最近 9 根内，任一持空 K 线的最高价与收盘价均在 KK–PP 压力带内，且收盘未上破 PP；榜单会列出全部回踩日期。</li><li><b>支撑回踩</b>：最近 9 根内，任一持多 K 线的最低价与收盘价均在 EE–DD 支撑带内，且收盘严格高于下沿 EE；榜单会列出全部回踩日期。</li></ul><p className="legend-note">报告中仍保留多转空、空转多两类转换预警，供策略分析使用；它们不显示在榜单预警区。</p></>}
      </section>
    </div>
  )
}
